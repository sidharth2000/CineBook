 ...
 * Description:
 * 
 * This class is to Initialise the Database tables with default values / example values
 * 
 */
...
import com.cinebook.discountcodefactory.PromoCodeCreator;
...
public class DataInitializer implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		// ------------------------------------- Fill RequestStatus table with
		// PENDING,APPROVED,REJECTED -- corresponds to Theatre Onboard Approval
		...
		// ------------------------------------- Fill User-Admin table with one Admin
		...
		if (languageRepository.count() == 0) {
			List<String> languages = Arrays.asList("English", "Irish", "Tamil", "Malayalam", "Chinese", "Hindi",
					"Swedish");
		...
		// ------------------------------------- Fill Voucher Types -- Marketplace
		// Product for user to buy with loyalty points
		...
		if (bookingStatusRepository.count() == 0) {
			...
			bookingStatusRepository.saveAll(Arrays.asList(pending, success, failure, cancelled));

			System.out.println("Booking statuses initialized!");
		}

		// ------------------------------------------ Fill Promocode with factory
		if (promoCodeRepository.count() == 0) {
			PromoCodeCreator christmasCreator = new PromoCodeCreator("CHRISTMAS2025", 20f, 50f,
					LocalDateTime.now().minusDays(1), LocalDateTime.now().plusDays(30), 3);
			PromoCode christmasPromo = (PromoCode) christmasCreator.generateCode();
			promoCodeRepository.save(christmasPromo);

			PromoCodeCreator newYearCreator = new PromoCodeCreator("NEWYEAR2025", 25f, 75f,
					LocalDateTime.now().minusDays(1), LocalDateTime.now().plusDays(45), 2);
			PromoCode newYearPromo = (PromoCode) newYearCreator.generateCode();
			promoCodeRepository.save(newYearPromo);

			System.out.println("Promo Codes initialized!");
		}

		// ------------------------------------------ Fill GlobalConfig - K-V pair
		if (globalConfigRepository.count() == 0) {
			GlobalConfig taxConfig = new GlobalConfig();
			taxConfig.setConfigKey("TAX_PERCENTAGE");
			taxConfig.setConfigValue("10.0");
			...
			loyaltyConfig.setConfigValue("0.1");
