...
public class LockSeatsCommand implements BookingCommand {
    ...
    public void execute() {
        ...
        for (Seat seat : seats) {
            List<Booking> existingBookings = bookingRepository.findByShowTimeAndSeatsContainingAndStatusIn(
                showTime, seat, List.of(pendingStatus, successStatus)
            );
        ...
        // Calculate total price exactly as before
        float totalPrice = 0f;
        for (Seat s : seats) {
            totalPrice += (s.getSeatCategory().getPriceMultiplier() * showTime.getPrice());
        }

        // Create Booking with PENDING status if not already provided
        if (booking == null) {
            booking = new Booking();
