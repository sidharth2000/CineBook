...
public class BookingServiceImpl implements BookingService {
	...
	public List<TheatreBrowseResponse> browseTheatres(BrowseTheatreRequest request) {
		...
		for (ShowTime st : showTimes) {
			...
			if (request.getSearch() != null && !request.getSearch().isBlank()) {
				if (!theatre.getTheatreName().toLowerCase().contains(request.getSearch().toLowerCase())) {
					continue;
				}
			}

			if (request.getCounty() != null && !request.getCounty().isBlank()) {
				if (!theatre.getAddress().getCounty().trim().equalsIgnoreCase(request.getCounty().trim())) {
					continue;
				}
			}

			if (!uniqueTheatres.containsKey(theatre.getTheatreId())) {
				TheatreBrowseResponse resp = new TheatreBrowseResponse(theatre.getTheatreId(), theatre.getTheatreName(),
						theatre.getOverview(), theatre.getAddress().getCounty());
	...
	public TheatreShowtimesResponse getShowtimesByTheatre(Long theatreId) {
		...
		List<ShowTime> showTimes = showTimeRepository.findByScreen_ScreenIdInAndStartTimeAfter(screenIds,
				LocalDateTime.now());
		...
		for (ShowTime st : showTimes) {
			...
			// adjust the method names here if your Format/Language classes use different
			// getters.
		...
		// 5. Convert nested maps to the DTO schedule Map<LocalDate,
		// List<MovieShowtimeGroup>>
		Map<java.time.LocalDate, List<TheatreShowtimesResponse.MovieShowtimeGroup>> schedule = new HashMap<>();
		for (Map.Entry<java.time.LocalDate, Map<Long, TheatreShowtimesResponse.MovieShowtimeGroup>> dateEntry : byDate
				.entrySet()) {
	...
	@Override
	public List<MovieBrowseResponse> getAvailableMovies(List<Long> languageIds, List<Long> formatIds, String genre,
		...
		if ((languageIds != null && !languageIds.isEmpty()) && (formatIds != null && !formatIds.isEmpty())) {
			...
			showTimes = showTimeRepository.findByStartTimeAfterAndLanguage_LanguageIdIn(LocalDateTime.now(),
					languageIds);
		} else if (formatIds != null && !formatIds.isEmpty()) {
			// Only format filter
			showTimes = showTimeRepository.findByStartTimeAfterAndFormat_FormatIdIn(LocalDateTime.now(), formatIds);
		} else {
			// No filters
	...
	public ShowTimeDetailsResponse getShowTimeDetails(Long showTimeId) {
		...
		List<Booking> bookings = bookingRepository.findByShowTime_ShowTimeIdAndStatus_StatusNameIn(showTimeId,
				List.of("SUCCESS", "PENDING"));

		Set<Long> bookedSeatIds = bookings.stream().flatMap(b -> b.getSeats().stream()).map(Seat::getSeatId)
	...
	public LockSeatsResponse lockSeats(Long showTimeId, String customerEmail, List<Long> seatIds) {
		...
		LockSeatsCommand lockCommand = new LockSeatsCommand(showTimeId, customerEmail, seatIds, bookingRepository,
				bookingStatusRepository, customerRepository, seatRepository, showTimeRepository, taxPercentage, null);
		...
		response.setLockedSeats(booking.getSeats().stream().map(Seat::getSeatName).toList());
	...
	public ApplyDiscountResponse applyDiscount(UUID bookingId, String discountCode, String customerEmail) {
		...
		// Create command
		ApplyDiscountCommand command = new ApplyDiscountCommand(bookingId, discountCode, customer, bookingRepository,
				bookingStatusRepository, promoCodeRepository, voucherCodeRepository);
	...
	public ApplyDiscountResponse undoDiscount(UUID bookingId, String customerEmail) {
		...
		ApplyDiscountCommand command = new ApplyDiscountCommand(bookingId, "", customer, bookingRepository,
				bookingStatusRepository, promoCodeRepository, voucherCodeRepository);
	...
	public ConfirmBookingResponse confirmBooking(UUID bookingId) {
		...
		System.out.println("Mock payment processed successfully for Booking ID " + bookingId + " Amount: "
		...
		ConformBookingCommand command = new ConformBookingCommand(bookingId, bookingRepository, bookingStatusRepository,
				loyaltyFactorValue);
		command.execute();
