/**
 * @author Mathew & Abhaydev
 * 
 * Description:
 * Onboarding Theatre API demands the creation of Theatre,Screen,Seat,SeatCategory,TheatreOnboardingRequest
 * from minimul detail DTO.
 * The Builder makes it easier to create all the entity classes from DTO in the service
 * 
 */
	...
	public TheatreBuilder screens(List<TheatreOnboardingRequest.ScreenRequest> screenRequests) {
		...
		for (TheatreOnboardingRequest.ScreenRequest screenReq : screenRequests) {
					...
					for (TheatreOnboardingRequest.SeatCategoryRequest catReq : screenReq.getSeatCategories()) {
						if (catReq.getRowsCovered().contains(row)) {
							Optional<SeatCategory> matched = seatCategories.stream()
									.filter(sc -> sc.getCategoryName().equals(catReq.getCategoryName())).findFirst();
			...
			for (SeatCategory sc : seatCategories) {
				List<Seat> catSeats = seats.stream().filter(s -> s.getSeatCategory() == sc).toList();
