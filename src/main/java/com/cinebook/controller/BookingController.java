...
public class BookingController {
	...
	public ResponseEntity<ApiResponse<List<TheatreBrowseResponse>>> browseTheatres(
	    ...
	    try {
	        ...
	        if (!isCustomer) {
	            ApiResponse<List<TheatreBrowseResponse>> error =
	                    new ApiResponse<>("failure", null, "Access denied: Customers only");
	        ...
	        ApiResponse<List<TheatreBrowseResponse>> response =
	                new ApiResponse<>("success", theatres, "Theatres retrieved successfully");
	...
	public ResponseEntity<ApiResponse<TheatreShowtimesResponse>> getShowtimesByTheatre(
	    ...
	    try {
	        ...
	        if (!isCustomer) {
	            ApiResponse<TheatreShowtimesResponse> error =
	                    new ApiResponse<>("failure", null, "Access denied: Customers only");
	            return ResponseEntity.status(HttpStatus.FORBIDDEN).body(error);
	        }

	        // --- Fetch showtimes using service ---
	        TheatreShowtimesResponse theatreShowtimes =
	                bookingService.getShowtimesByTheatre(theatreId);

	        ApiResponse<TheatreShowtimesResponse> response =
	                new ApiResponse<>("success", theatreShowtimes, "Showtimes retrieved successfully");
	...
	public ResponseEntity<ApiResponse<List<MovieBrowseResponse>>> browseMovies(
	    ...
	    try {
	        ...
	        if (!isCustomer) {
	            ApiResponse<List<MovieBrowseResponse>> error =
	                    new ApiResponse<>("failure", null, "Access denied: Customers only");
	            return ResponseEntity.status(HttpStatus.FORBIDDEN).body(error);
	        }

	        // --- Fetch movies ---
	        List<MovieBrowseResponse> movies = bookingService.getAvailableMovies(
	                request.getLanguageIds(),
	                request.getFormatIds(),
	                request.getGenre(),
	                request.getSearch(),
	                request.isSortAsc()
	        );

	        ApiResponse<List<MovieBrowseResponse>> response =
	...
	public ResponseEntity<ApiResponse<ShowTimeDetailsResponse>> getShowTimeDetails(
	    ...
	    try {
	        ...
	        return ResponseEntity.ok(
	                new ApiResponse<>("success", data, "Showtime details retrieved")
	        );
    ...
    public ResponseEntity<ApiResponse<LockSeatsResponse>> lockSeats(
        ...
        try {
            ...
            LockSeatsResponse response = bookingService.lockSeats(request.getShowTimeId(), customerEmail, request.getSeatIds());
